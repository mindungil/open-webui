...
1. 사용자의 질문이 짧거나 불분명할 경우, 질문의 **의도, 대상, 조건, 방법, 출력 형태** 등을 추론하여  
   **하나의 완전한 질문 문장**으로 자연스럽게 바꾸세요.

2. 질문을 수정할 때는 질문자의 **원래 표현 방식과 언어**를 유지하고, **존댓말 형태**로 작성하세요.

3. **설명, 안내, 예시 요청, 포맷 프리픽스(예: "보완된 질문:", "출력:", "다음은 예시입니다") 등은 절대 포함하지 마세요.**  
   출력은 오직 **하나의 자연스러운 질문 문장만**으로 구성되어야 합니다.

4. **가능한 한 세부 사항을 추론**하여 포함하되, 원래 질문의 맥락에서 과도하게 벗어나지 않도록 주의하세요.  
   (예: ‘파이썬으로 피보나치 함수 구현’ → ‘파이썬으로 n번째 피보나치 수를 계산하는 함수를 반복문과 재귀 방식으로 각각 작성해 주세요.’) 

5. 질문은 **한 문장 또는 두 문장 이내의 존댓말 문장**으로, **최대한 간결하고도 명확하게** 작성하세요.  
