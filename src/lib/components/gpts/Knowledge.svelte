<script lang="ts">
	...
	const deleteHandler = async (item) => {
		...
		if (res) {
			knowledgeBases = await getKnowledgeBaseList(localStorage.token, 'gpts');
			knowledge.set(await getKnowledgeBases(localStorage.token, 'gpts'));
...
{#if loaded}
		...
		<div class="flex justify-between items-center">
			...
			<div class="flex w-full justify-end gap-1.5">
				<a
					...
					href="/gpts/knowledge/create"
				>
					<Plus className="size-3" strokeWidth="2.5" />

					<div class=" hidden md:block md:ml-1 text-xs">새 지식 데이터</div>
				</a>
			</div>
		</div>
	</div>
						...
						<button
							...
							on:click={() => {
								if (item?.meta?.document) {
									toast.error(
										'컬렉션만 편집할 수 있습니다. 문서를 편집/추가하려면 새 지식 데이터를 생성하세요.'
									);
								} else {
									goto(`/gpts/knowledge/${item.id}`);
								}
							}}
						>
							<div class=" w-full">
		...
		{:else}
			<div class=" w-full h-full flex flex-col justify-center items-center my-16 mb-24">
				<div class="max-w-md text-center">
					...
					<div class=" text-lg font-medium mb-1">지식 데이터를 찾을 수 없습니다</div>
	...
	<div class=" text-gray-500 text-xs m-2">
		ⓘ 프롬프트 입력에서 '#'를 사용하여 지식 데이터를 불러오고 포함하세요.
	</div>
{:else}
