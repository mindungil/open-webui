<script>
	...
	const submitHandler = async () => {
		...
		if (res) {
			toast.success('성공적으로 지식 데이터가 생성되었습니다');
			knowledge.set(await getKnowledgeBases(localStorage.token, 'gpts'));
			goto(`/gpts/knowledge/${res.id}`);
		}
				...
				<div>
					...
					<div class=" w-full mt-1">
						<textarea
							...
							placeholder="지식 데이터에 대한 설명과 목적을 입력하세요"
							required
						/>
					</div>
				</div>
		...
		<div class="flex justify-end mt-2">
			<div>
				<button
					...
					disabled={loading}
				>
					<div class=" self-center font-medium">지식 데이터 생성</div>
